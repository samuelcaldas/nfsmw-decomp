export const meta = {
  name: 'nfsmw-parallel-decomp-batch13',
  description: 'Batch 13 decompilation: CalculateRegionInfo, InitAtSegment, UpdatePursuits, AssignClosestOffsets',
  phases: [
    { title: 'Decomp-1', detail: 'Pair 1: CalculateRegionInfo (zTrack) & InitAtSegment (zWorld2)' },
    { title: 'Decomp-2', detail: 'Pair 2: UpdatePursuits (zAI) & AssignClosestOffsets (zAI)' },
...
const CANDIDATES_PAIR1 = [
  {
    ...
    hint: 'Around lines 137-142 in WeatherMan.cpp, adjust assignment order: this->DistFogStart = start; before this->FogFalloff = falloff; so that stfs f11, 0xc(r3) is stored first.',
  },
  {
    ...
    hint: 'Around line 2435 in WRoadNetwork.cpp, check register allocation and local variable usage. Target saves r24..r31 vs current r25..r31. Adjust temporary variables to match stack frame (-0x90 vs -0x98).',
  },
]
...
const CANDIDATES_PAIR2 = [
  {
    ...
    hint: 'Around line 1836 in AICopManager.cpp. Function has 100% assembly instruction match with only r25/r26 register swap (VehicleClass::CHOPPER vs 0x4330 constant). Align local variable declaration order to match registers.',
  },
  {
    ...
    hint: 'Around lines 1134-1204 in AIPursuit.cpp. Study decomp-diff: check register allocation (r28 vs r29) and loop test condition (mr. r9, r28 vs cmpwi).',
  },
]
...
log('Starting Batch 13: CalculateRegionInfo, InitAtSegment, UpdatePursuits, AssignClosestOffsets')
...
if (mergedFunctions.length > 0) {
  ...
  await agent(
    ...
    '   git commit -m "docs: document Batch 13 decompiled functions (@nfs1)\n\nCo-Authored-By: Claude Fable 5 <noreply@anthropic.com>"\n' +
