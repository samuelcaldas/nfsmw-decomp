export const meta = {
  name: 'nfsmw-parallel-decomp-batch14',
  description: 'Batch 14 decompilation: GetLoadingPriority, CookieTrailCurvature, InternalLoadStreamingEntry, RenderVisibleSectionBoundary',
  phases: [
    { title: 'Decomp-1', detail: 'Pair 1: GetLoadingPriority (zTrack) & CookieTrailCurvature (zWorld2)' },
    { title: 'Decomp-2', detail: 'Pair 2: InternalLoadStreamingEntry (zEcstasy) & RenderVisibleSectionBoundary (zTrack)' },
...
const CANDIDATES_PAIR1 = [
  {
    ...
    hint: 'Around line 2145 in TrackStreamer.cpp. Notice stack frame allocation difference (-0x120 target vs -0x80 compiled). Look at lines 2182-2210 where code is inside #if 0 or local variables were declared. Check register allocation: target saves r28..r31 and f29..f31, uses f31 for speed comparison.',
  },
  {
    demangled: 'WRoadNav::CookieTrailCurvature',
    symbol: 'CookieTrailCurvature__8WRoadNavRCQ25UMath7Vector3T1',
    unit: 'main/Speed/Indep/SourceLists/zWorld2',
    size: 1284,
    match: 95.94,
    source_file: 'src/Speed/Indep/Src/World/Common/WRoadNetwork.cpp',
    virtual_address: '0x80304A70',
    signature: 'float WRoadNav::CookieTrailCurvature(const UMath::Vector3 &car_position, const UMath::Vector3 &car_velocity)',
    hint: 'Around line 3263 in WRoadNetwork.cpp. Stack frame is -0x78 in target vs -0x70 in compiled. Target saves f25..f31 vs f26..f31. In line 3265, road_curvature is initialized to 0.0f; check intermediate temporary variables and accumulator float registers.',
  },
]
...
const CANDIDATES_PAIR2 = [
  {
    ...
    hint: 'Around line 336 in eStreamingPack.cpp. Check parameter saving: target does mr r29, r5 vs mr r28, r5. Look at refcount checks and malloc_size calculations.',
  },
  {
    demangled: 'RenderVisibleSectionBoundary',
    symbol: 'RenderVisibleSectionBoundary__FP22VisibleSectionBoundaryP5eView',
    unit: 'main/Speed/Indep/SourceLists/zTrack',
    size: 888,
    match: 95.43,
    source_file: 'src/Speed/Indep/Src/World/Scenery.cpp',
    virtual_address: '0x802BE498',
    signature: 'void RenderVisibleSectionBoundary(VisibleSectionBoundary *boundary, eView *view)',
    hint: 'Around line 591 in Scenery.cpp. Check loop boundary in line 592: cast boundary->GetNumPoints() to char/int, and check WorldTimer.GetSeconds() conversion in line 598.',
  },
]
...
log('Starting Batch 14: GetLoadingPriority, CookieTrailCurvature, InternalLoadStreamingEntry, RenderVisibleSectionBoundary')
...
if (mergedFunctions.length > 0) {
  ...
  await agent(
    ...
    '   git commit -m "docs: document Batch 14 decompiled functions (@nfs1)\n\nCo-Authored-By: Claude Fable 5 <noreply@anthropic.com>"\n' +
