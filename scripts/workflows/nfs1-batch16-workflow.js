export const meta = {
  name: 'nfsmw-parallel-decomp-batch16',
  description: 'Batch 16 decompilation: CalculateRegionInfo, UpdatePursuits, InitAtSegment, InternalLoadingHeaderPhase2Callback',
  phases: [
    { title: 'Decomp-1', detail: 'Pair 1: CalculateRegionInfo (zTrack) & UpdatePursuits (zAI)' },
    { title: 'Decomp-2', detail: 'Pair 2: InitAtSegment (zWorld2) & InternalLoadingHeaderPhase2Callback (zEcstasy)' },
...
const CANDIDATES_PAIR1 = [
  {
    ...
    hint: 'Around line 119-145 in WeatherMan.cpp. 99.9% match. Only 4 instructions differ: store order of float registers (f11 vs f12, f13, f0) to the output struct at offset 0xc vs 0, 4, 8 around lines 137-142. Adjust assignment order.',
  },
  {
    ...
    hint: 'Around line 1836 in AICopManager.cpp. 100.0% equivalent instructions, only minor register swap between r25 and r26 for 0x4330 float bias and VehicleClass::CHOPPER. Adjust local variable declarations/ordering.',
  },
]
...
const CANDIDATES_PAIR2 = [
  {
    ...
    signature: 'void WRoadNav::InitAtSegment(short segment_index, char lane, float t)',
    hint: 'Around line 2435 in WRoadNetwork.cpp. Stack frame is -0x90 in target vs -0x98 in compiled. Register saves are stmw r24 vs stmw r25. Notice parameter lane is placed into r24. Adjust local variables and eliminate extra stack temp to reduce frame to -0x90.',
  },
  {
    ...
    hint: 'Around line 804 in eStreamingPack.cpp. Stack frame is exact (-0x440). Look at lines 818-824: adjust assignment ordering of StreamingEntryTable, LoadResourceFileAmount, StreamingEntryNumEntries, SolidListHeader, and pTexturePackHeader to match target load/store instruction sequence.',
  },
]
...
log('Starting Batch 16: CalculateRegionInfo, UpdatePursuits, InitAtSegment, InternalLoadingHeaderPhase2Callback')
...
if (mergedFunctions.length > 0) {
  ...
  await agent(
    ...
    '   git commit -m "docs: document Batch 16 decompiled functions (@nfs1)\n\nCo-Authored-By: Claude Fable 5 <noreply@anthropic.com>"\n' +
