export const meta = {
  name: 'nfsmw-parallel-decomp-batch19',
  description: 'Batch 19 decompilation: UpdatePursuits (zAI), FindFaceInCInst (zWorld2), CalculateRegionInfo (zTrack), __InitMatrices (zEcstasy)',
  phases: [
    { title: 'Decomp-1', detail: 'Pair 1: UpdatePursuits (zAI) & FindFaceInCInst (zWorld2)' },
    { title: 'Decomp-2', detail: 'Pair 2: CalculateRegionInfo (zTrack) & __InitMatrices (zEcstasy)' },
...
const CANDIDATES_PAIR1 = [
  {
    ...
    hint: 'Around line 1836 in src/Speed/Indep/Src/AI/Activities/AICopManager.cpp. Function has 99.97% match (100.0% fuzzy match). The only diff is r25 and r26 swapped between 0x4330 float conversion constant and VehicleClass::CHOPPER reference at lines 1840/1870. Adjusting the order of initialization/references aligns register assignments to 100.0% exact match.',
  },
  {
    ...
    hint: 'Around line 275 in src/Speed/Indep/Src/World/Common/WCollisionMgr.cpp. Initial match is 99.92%. Only slight register allocation difference on stack addresses 0x98, 0xa8, 0xb8 (r26, r28, r27) for local matrices/transforms around lines 277-289. Adjusting declaration order aligns registers to 100.0% match.',
  },
]

const CANDIDATES_PAIR2 = [
  {
    demangled: 'RegionQuery::CalculateRegionInfo',
    ...
    hint: 'Around line 119 in src/Speed/Indep/Src/World/WeatherMan.cpp. Initial match is 99.92%. In FogControlOverRide block (lines 137-142), target writes DistFogStart (0xc), FogFalloff (0x0), FogFalloffX (0x4), FogFalloffY (0x8). Adjusting the assignment order of DistFogStart, FogFalloff, FogFalloffX, FogFalloffY aligns stores to 100.0% exact match.',
  },
  {
    ...
    hint: 'Around line 680 in src/Speed/GameCube/Src/Ecstasy/EcstasyE.cpp. Initial match is 98.0%. In target, scale_x = gcn_scale * (448.f / 378.f) is calculated early and reused across transx and MTXScale. Check local float expressions ordering and assignment around lines 687-691.',
  },
]
...
log('Starting Batch 19: UpdatePursuits (zAI), FindFaceInCInst (zWorld2), CalculateRegionInfo (zTrack), __InitMatrices (zEcstasy)')
...
if (mergedFunctions.length > 0) {
  ...
  await agent(
    ...
    '5. Update header summary in docs/decompilation_status_ledger.md to mention Batch 19.\n' +
    '6. Commit the docs:\n' +
    '   git add docs/decompiled_functions.md docs/decompilation_status_ledger.md\n' +
    '   git commit -m "docs: document Batch 19 decompiled functions (@nfs1)\n\nCo-Authored-By: Claude Fable 5 <noreply@anthropic.com>"\n\n' +
