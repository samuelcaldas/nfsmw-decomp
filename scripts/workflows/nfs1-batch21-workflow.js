export const meta = {
  name: 'nfsmw-parallel-decomp-batch21',
  description: 'Batch 21 parallel decompilation: AICopManager::UpdatePursuits (zAI), GinsuSynthesis::HandlePacketRelease (zEAXSound2), CarPartCuller::CullParts (zWorld), EmitterSystem::UpdateParticles (zEcstasy), MemcardCallbacks::FoundEntry (zFe), UIQRCarSelect::SetupForPlayer (zFeOverlay)',
  phases: [
    { title: 'Decomp-P1', detail: 'Trio 1: UpdatePursuits (zAI), HandlePacketRelease (zEAXSound2), CullParts (zWorld)' },
    { title: 'Decomp-P2', detail: 'Trio 2: UpdateParticles (zEcstasy), FoundEntry (zFe), SetupForPlayer (zFeOverlay)' },
...
const TRIO_1 = [
  {
    ...
    hint: 'Around line 1836 in src/Speed/Indep/Src/AI/Activities/AICopManager.cpp. Initial match is 99.97% (100.0% fuzzy match, 4 mismatched instructions). In lines 1837-1840 vs 1870-1874: in decomp, VehicleClass::CHOPPER uses r25 and the float conversion constant 0x4330 uses r26, while the target binary has them swapped (r25 is 0x4330, r26 is CHOPPER). Adjust local variable declaration order or initialization order at top of UpdatePursuits so 0x4330 is assigned to r25 and CHOPPER to r26.',
  },
  {
    ...
    hint: 'Around line 25 in src/Speed/Indep/Src/EAXSound/Ginsu/ginsusynth.cpp. Initial match is 99.54%. Around lines 92-96: differences are register swap between r8 and r10 for sample index and 0x4330 float constant, and float addition scheduling (lfs f12). Reorder variable declarations and loop indexing.',
  },
  {
...
const TRIO_2 = [
  ...
  },
  {
    ...
    hint: 'Around line 237 in src/Speed/Indep/Src/Frontend/MemoryCard/MemoryCardCallbacks.cpp. Initial match is 98.0%. In lines 256-263 (GetScreen()->AddItem(info->mName, "", gs, fd);): differences are the scheduling of loading empty string constant "" ($LC71) and parameter setup for AddItem. Reorder temporary variable declarations or expression statements to match instruction sequence.',
  },
  {
    demangled: 'UIQRCarSelect::SetupForPlayer',
    symbol: 'SetupForPlayer__13UIQRCarSelecti',
    ...
    virtual_address: '0x803ACFB8',
    signature: 'void UIQRCarSelect::SetupForPlayer(int player)',
    hint: 'Around line 926 in src/Speed/Indep/Src/Frontend/MenuScreens/Safehouse/quickrace/uiQRCarSelect.cpp. Initial match is 95.9%. In loop (lines 930-951), car = sc; was placed before checking conditions causing early register move mr r29, r31. In target binary, car is assigned only upon match (car = sc; break;), generating mr r29, r31; b 0x8de0 at the match exit. Adjust loop structure to match target control flow.',
  },
]
...
log('Starting Batch 21 (6 candidates across 6 units)')
...
if (mergedFunctions.length > 0) {
  ...
  await agent(
    ...
    '5. Update header summary in docs/decompilation_status_ledger.md to mention Batch 21.\n' +
    '6. Commit the docs:\n' +
    '   git add docs/decompiled_functions.md docs/decompilation_status_ledger.md\n' +
    '   git commit -m "docs: document Batch 21 decompiled functions (@nfs1)\n\nCo-Authored-By: Claude Fable 5 <noreply@anthropic.com>"\n\n' +
