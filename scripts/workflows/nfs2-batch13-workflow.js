export const meta = {
  name: 'nfs2-batch13-decomp',
  description: 'Batch 13 decompilation for @nfs2 units: bASin (zBWare), FEGroup::~FEGroup (zFEng), SFXCTL_Physics::UpdateMixerOutputs (zEAXSound), CarLoader::SetMemoryPoolSize (zWorld)',
  phases: [
...
const CANDIDATES = [
  {
    ...
    description: 'Computes arcsine using fast lookup table and fixed-point interpolation.',
    hint: 'Currently 92.32% match (336 bytes). Differences are in register assignments during quadrant reduction and fixed-point scaling (r10/r8 swap, r0/r7 swap) and loop branching around lines 275-295. Align local variable ordering and temporary registers to reach 100.0%.',
  },
  {
    demangled: 'FEGroup::~FEGroup(void)',
    symbol: '_._7FEGroup',
    diff_filter: '_._7FEGroup',
    unit: 'main/Speed/Indep/SourceLists/zFEng',
    unit_obj: 'build/GOWE69/src/Speed/Indep/SourceLists/zFEng.o',
    size: 76,
    match: 84.16,
    source_file: 'src/Speed/Indep/Src/FEng/FEGroup.h',
    virtual_address: '0x80190AFC',
    signature: 'FEGroup::~FEGroup()',
    description: 'Destructor for frontend object group hierarchy container.',
    hint: 'Currently 84.16% match (76 bytes). Destructor in FEGroup.h currently stores the FEGroup vtable at offset 8, whereas original binary does not assign the FEGroup vtable (only FEMinList vtable at offset 0x68 before calling FEMinList::Purge). Match destructor pattern without FEGroup vtable reassignment.',
  },
  {
    ...
    description: 'Calculates physics audio mixer output levels from suspension and velocity magnitudes.',
    hint: 'Currently 98.22% match (1212 bytes). Diff is in constant address loading: lis r7, $LC@ha is hoisted across vector magnitude calculations in the target binary, while compiled version re-loads or uses r9. Hoist the constant load or cache it in a local pointer to achieve 100.0%.',
  },
  {
    ...
    description: 'Configures car loader heap memory pool allocation size.',
    hint: 'Currently 97.37% match (304 bytes). Exactly one instruction scheduling difference: stw r0, CarLoaderMemoryPoolNumber@l(r29) is emitted right before bl bInitMemoryPool in the original binary, whereas the compiled code emits it two instructions earlier. Adjust compiler barrier or statement ordering to reach 100.0%.',
  },
]

log('Starting Batch 13 parallel decomp for ' + CANDIDATES.length + ' candidates across @nfs2 units')
...
if (mergedFunctions.length > 0) {
  ...
  await agent(
    ...
    '   git commit -m "docs: document Batch 13 decompiled functions (@nfs2)\n\nCo-Authored-By: Claude Code <noreply@anthropic.com>"\n' +
