export const meta = {
  name: 'nfs2-batch14-decomp',
  description: 'Batch 14 decompilation for @nfs2 units: bATan (zBWare), FEQuaternion::operator* (zFEng), EAXCommon::Play (zEAXSound), CarLoader::DefragmentPool (zWorld)',
  phases: [
...
const CANDIDATES = [
  {
    ...
    signature: 'bAngle bATan(float y, float x)',
    description: 'Computes arctangent of y/x using fast lookup table and fixed-point interpolation.',
    hint: 'Currently 98.05% match (328 bytes). Difference is in register assignments (r10 vs r11, r6 vs r10) during fixed-point scaling and array indexing in quadrants 1 and 2. Reorder local variables or intermediate assignments in lines 344-353 to achieve 100.0%.',
  },
  {
    demangled: 'FEQuaternion::operator*(const FEQuaternion &)',
    symbol: '__ml__12FEQuaternionRC12FEQuaternion',
    diff_filter: '__ml__12FEQuaternionRC12FEQuaternion',
    ...
    description: 'Multiplies two quaternions representing 3D spatial rotations.',
    hint: 'Currently 89.06% match (212 bytes). Register allocation and floating-point multiply-accumulate scheduling difference in quaternion product formula in FEMath.h lines 58-66. Align intermediate term ordering to match GOWE69 compiled code.',
  },
  {
    ...
    description: 'Plays front-end UI audio menu sound trigger.',
    hint: 'Currently 96.83% match (492 bytes). Minor branch and return value register scheduling (r3 vs r9) around Debug_Common_FE_OFF check at lines 196-204 in EAXCommon.cpp. Adjust early return / condition logic to match.',
  },
  {
    demangled: 'CarLoader::DefragmentPool',
    symbol: 'DefragmentPool__9CarLoader',
    diff_filter: 'DefragmentPool__9CarLoader',
    ...
    virtual_address: '0x802DFCAC',
    signature: 'int CarLoader::DefragmentPool()',
    description: 'Defragments vehicle model memory pool by compacting allocations and updating pointers via DefragFixer.',
    hint: 'Currently 92.47% match (684 bytes). Differences in register allocation for local pointers (r15 vs r14 in prologue/epilogue stmw/lmw) and loop index temporary registers in CarLoader.cpp around lines 1933-2021.',
  },
]

log('Starting Batch 14 parallel decomp for ' + CANDIDATES.length + ' candidates across @nfs2 units')
...
if (mergedFunctions.length > 0) {
  ...
  await agent(
    ...
    '   git commit -m "docs: document Batch 14 decompiled functions (@nfs2)\n\nCo-Authored-By: Claude Code <noreply@anthropic.com>"\n' +
