export const meta = {
  name: 'nfs2-batch17-decomp',
  description: 'Batch 17 decompilation for @nfs2 units: UpdateWheelYRenderOffset (zWorld), ScrollSelection (zFEng), GetMatrix (zFEng), fDeterminant (zBWare)',
  phases: [
...
const CANDIDATES = [
  {
    demangled: 'CarRenderInfo::UpdateWheelYRenderOffset()',
    symbol: 'UpdateWheelYRenderOffset__13CarRenderInfo',
    diff_filter: 'UpdateWheelYRenderOffset__13CarRenderInfo',
    ...
    virtual_address: '0x802C9FB8',
    signature: 'void CarRenderInfo::UpdateWheelYRenderOffset()',
    description: 'Updates wheel Y render offsets and scale factors based on ride info and attributes.',
    hint: 'Currently 95.2% match (876 bytes). Differences are minor stack frame offset / register allocation (r25 vs r26, r14 vs r15) in CarRender.cpp around lines 1925-2008.',
  },
  {
    demangled: 'FEListBox::ScrollSelection(long, long)',
    symbol: 'ScrollSelection__9FEListBoxll',
    diff_filter: 'ScrollSelection__9FEListBoxll',
    ...
    signature: 'void FEListBox::ScrollSelection(long lColumnNum, long lRowNum)',
    description: 'Scrolls selection by column and row deltas, updating listbox item offsets and wrapping.',
    hint: 'Currently 96.5% match (1104 bytes). Register allocation differences (r7 vs r11, r8 vs r9) in FEListBox.cpp around lines 178-290.',
  },
  {
    demangled: 'FEQuaternion::GetMatrix(FEMatrix4 *)',
    symbol: 'GetMatrix__12FEQuaternionP9FEMatrix4',
    diff_filter: 'GetMatrix__12FEQuaternionP9FEMatrix4',
    unit: 'main/Speed/Indep/SourceLists/zFEng',
    unit_obj: 'build/GOWE69/src/Speed/Indep/SourceLists/zFEng.o',
    size: 196,
    match: 88.6,
    ...
    description: 'Converts a unit quaternion orientation to a 4x4 transformation matrix.',
    hint: 'Currently 88.6% match (196 bytes). Operation ordering and store scheduling of matrix components in FEMath.cpp lines 4-37.',
  },
  {
    demangled: 'fDeterminant(bMatrix4*)',
    symbol: 'fDeterminant__FP8bMatrix4',
    diff_filter: 'fDeterminant__FP8bMatrix4',
    ...
    description: 'Computes the determinant of a 4x4 matrix using Laplace expansion.',
    hint: 'Currently 69.0% match (448 bytes). Grouping and evaluation order of 4x4 cofactor products in bMatrix.cpp around lines 13-33.',
  },
]

log('Starting Batch 17 parallel decomp for ' + CANDIDATES.length + ' candidates across @nfs2 units')
...
if (mergedFunctions.length > 0) {
  ...
  await agent(
    ...
    '   git commit -m "docs: document Batch 17 decompiled functions (@nfs2)\n\nCo-Authored-By: Claude Code <noreply@anthropic.com>"\n' +
