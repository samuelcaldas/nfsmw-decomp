export const meta = {
  name: 'nfs2-batch18-decomp',
  description: 'Batch 18 decompilation for @nfs2 units: bASin (zBWare), UpdateMixerOutputs (zEAXSound), SortObjects (zFEng), CullParts (zWorld)',
  phases: [
...
const CANDIDATES = [
  {
    ...
    description: 'Computes the arcsine using an interpolation table and binary search.',
    hint: 'Currently 92.3% match (336 bytes). Local variable register assignment (r8 vs r10) and instruction scheduling around bASinTable lookup in bMath.cpp lines 274-311.',
  },
  {
    demangled: 'SFXCTL_Physics::UpdateMixerOutputs',
    symbol: 'UpdateMixerOutputs__14SFXCTL_Physics',
    diff_filter: 'SFXCTL_Physics::UpdateMixerOutputs',
    ...
    hint: 'Currently 98.2% match (1212 bytes). Minor constant loading register allocation (r7 vs r9) for vector normalization in SFXCTL_Physics.cpp lines 147-203.',
  },
  {
    demangled: 'FEObjectSorter<1024>::SortObjects(void)',
    symbol: 'SortObjects__t14FEObjectSorter1i1024',
    diff_filter: 'SortObjects',
    ...
    virtual_address: '0x8019039c',
    signature: 'template <> void FEObjectSorter<1024>::SortObjects()',
    description: 'Sorts UI objects by rendering depth and priority into buckets via radix sort.',
    hint: 'Currently 84.0% match (316 bytes). In FEObjectSorter.h line 59, fix alElemIndex[pucByte[i]]++ to alElemCount[pucByte[i]]++ to match the radix sort counting histogram.',
  },
  {
    demangled: 'CarPartCuller::CullParts(bVector3 *, unsigned short)',
    symbol: 'CullParts__13CarPartCullerP8bVector3Us',
    diff_filter: 'CullParts',
    ...
    description: 'Culls car parts against view frustum planes based on camera eye position and steering angle.',
    hint: 'Currently 99.4% match (836 bytes, 209 instructions). Only 5 bytes difference: single constant loading order (lis r17 vs r18) in CarRender.cpp lines 269-351.',
  },
]

log('Starting Batch 18 parallel decomp for ' + CANDIDATES.length + ' candidates across @nfs2 units')
...
if (mergedFunctions.length > 0) {
  ...
  await agent(
    ...
    '   git commit -m "docs: document Batch 18 decompiled functions (@nfs2)\n\nCo-Authored-By: Claude Code <noreply@anthropic.com>"\n' +
