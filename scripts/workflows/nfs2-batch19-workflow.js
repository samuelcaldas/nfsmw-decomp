export const meta = {
  name: 'nfs2-batch19-decomp',
  description: 'Batch 19 decompilation for @nfs2 units: bATan (zBWare), Play (zEAXSound), Update (zFEng), UpdateLightStateTextures (zWorld)',
  phases: [
...
const CANDIDATES = [
  {
    ...
    hint: 'Currently 97.4% match (328 bytes). Minor register allocation differences (r8 vs r6, r11 vs r10) around bFastATanTable access in bMath.cpp lines 325-370.',
  },
  {
    demangled: 'EAXCommon::Play(eMenuSoundTriggers)',
    symbol: 'Play__9EAXCommon18eMenuSoundTriggers',
    diff_filter: 'EAXCommon::Play',
    ...
    description: 'Dispatches UI menu sound effects via DMIX outputs and Csis PlayCommonSample handles.',
    hint: 'Currently 96.8% match (492 bytes). Minor branch ordering and return value registers (r3 vs r9) for error conditions in EAXFrontEnd.cpp lines 185-225.',
  },
  {
    demangled: 'FEngine::Update(long, unsigned int)',
    symbol: 'Update__7FEnginelUi',
    diff_filter: 'FEngine::Update',
    ...
    description: 'Main UI engine update routine processing input pads, mouse, package hierarchies, and message queues.',
    hint: 'Currently 98.0% match (776 bytes). Register allocation (r30 vs r31, r25 vs r9) in pad hold decrement loop and dirty flag handling in FEngine.cpp lines 275-345.',
  },
  {
    ...
    description: 'Updates vehicle headlight, taillight, and glass replacement textures based on vehicle lighting states.',
    hint: 'Currently 96.2% match (1068 bytes). Register allocation and temporary local variables for replacement texture hashes in CarRender.cpp lines 2110-2185.',
  },
]

log('Starting Batch 19 parallel decomp for ' + CANDIDATES.length + ' candidates across @nfs2 units')
...
if (mergedFunctions.length > 0) {
  ...
  await agent(
    ...
    '   git commit -m "docs: document Batch 19 decompiled functions (@nfs2)\n\nCo-Authored-By: Claude Code <noreply@anthropic.com>"\n' +
