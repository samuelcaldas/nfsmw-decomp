export const meta = {
  name: 'nfs2-batch21-decomp',
  description: 'Batch 21 decompilation for @nfs2 units: RegionQuery::CalculateRegionInfo (zTrack), WCollisionMgr::FindFaceInCInst (zWorld2), SFXCTL_Physics::UpdateMixerOutputs (zEAXSound), TrackStreamer::GetLoadingPriority (zTrack), WRoadNav::UpdateOccludedPosition (zWorld2), RenderVisibleSectionBoundary (zTrack)',
  phases: [
...
const CANDIDATES = [
  {
    ...
    hint: 'Currently 99.92% match (1208 bytes, 302 instructions). In WeatherMan.cpp lines 135-143: reorder float assignments to align floating-point register stores f11, f12, f13, f0. Specifically, assign this->FogFalloffY = falloffY before this->DistFogStart = start.',
  },
  {
    ...
    hint: 'Currently 96.75% match (1436 bytes, 362 instructions). In WCollisionMgr.cpp lines 276-292: adjust argument copies and local matrix temporary assignments (r28 vs r24, r30 vs r29) to match register scheduling.',
  },
  {
    ...
    hint: 'Currently 98.65% match (1212 bytes, 304 instructions). In SFXCTL_Physics.cpp around line 157-170: adjust MixerSqrt inline branch so zero value loads directly as a float constant (0.0f) from rodata rather than via MixerSqrtZero.',
  },
  {
    demangled: 'TrackStreamer::GetLoadingPriority(TrackStreamingSection *, StreamingPositionEntry *, bool)',
    symbol: 'GetLoadingPriority__13TrackStreamerP21TrackStreamingSectionP22StreamingPositionEntryb',
    diff_filter: 'GetLoadingPriority',
    unit: 'main/Speed/Indep/SourceLists/zTrack',
    unit_obj: 'build/GOWE69/src/Speed/Indep/SourceLists/zTrack.o',
    size: 708,
    match: 97.01,
    source_file: 'src/Speed/Indep/Src/World/TrackStreamer.cpp',
    virtual_address: '0x802bc0a4',
    signature: 'int TrackStreamer::GetLoadingPriority(TrackStreamingSection *tss, StreamingPositionEntry *spe, bool flag)',
    description: 'Calculates streaming section load priority based on vehicle distance and heading angle.',
    hint: 'Currently 97.01% match (708 bytes, 177 instructions). In TrackStreamer.cpp lines 2147-2185: target stack frame size is 0x120 bytes (currently 0x80); add appropriate local buffer/struct to match stack allocation and check f31 vs f30 register assignment.',
  },
  {
    demangled: 'WRoadNav::UpdateOccludedPosition(bool)',
    symbol: 'UpdateOccludedPosition__8WRoadNavb',
    diff_filter: 'UpdateOccludedPosition',
    unit: 'main/Speed/Indep/SourceLists/zWorld2',
    unit_obj: 'build/GOWE69/src/Speed/Indep/SourceLists/zWorld2.o',
    size: 4368,
    match: 98.12,
    source_file: 'src/Speed/Indep/Src/World/Common/RoadNav.cpp',
    virtual_address: '0x802e3a10',
    signature: 'void WRoadNav::UpdateOccludedPosition(bool arg)',
    description: 'Updates vehicle navigation positioning accounting for occlusion and road boundaries.',
    hint: 'Currently 98.12% match (4368 bytes, 1101 instructions). In RoadNav.cpp lines 1921-2170: adjust local variable ordering and temporary bVector2 stack layouts around lines 2075-2090 to align register assignments.',
  },
  {
    ...
    diff_filter: 'RenderVisibleSectionBoundary',
    unit: 'main/Speed/Indep/SourceLists/zTrack',
    unit_obj: 'build/GOWE69/src/Speed/Indep/SourceLists/zTrack.o',
    size: 888,
    match: 95.43,
    source_file: 'src/Speed/Indep/Src/World/WeatherMan.cpp',
    virtual_address: '0x802c2f70',
    signature: 'void RenderVisibleSectionBoundary(VisibleSectionBoundary *boundary, eView *view)',
    description: 'Renders visible boundary debug lines and portals between track streaming sections.',
    hint: 'Currently 95.43% match (888 bytes, 224 instructions). In WeatherMan.cpp lines 591-628: adjust loop variable caching and register allocation around WorldTimer float conversion and color interpolation.',
  },
]

log('Starting Batch 21 parallel decomp for ' + CANDIDATES.length + ' candidates across @nfs2 units')
...
if (mergedFunctions.length > 0) {
  ...
  await agent(
    ...
    '   git commit -m "docs: document Batch 21 decompiled functions (@nfs2)\n\nCo-Authored-By: Claude Fable 5 <noreply@anthropic.com>"\n' +
