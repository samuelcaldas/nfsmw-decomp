export const meta = {
  name: 'nfs2-batch5-decomp',
  description: 'Batch 5 decompilation for @nfs2 units: bATan (zBWare), FEngine::Update (zFEng), EAXCommon::Play (zEAXSound), CarPartCuller::CullParts (zWorld)',
  phases: [
...
const CANDIDATES = [
  {
    ...
    virtual_address: '0x8005CD44',
    signature: 'bAngle bATan(float y, float x)',
    description: 'Computes arc tangent using fast table lookup and slope interpolation.',
    hint: 'Register allocation difference between r6 and r7 in quadrant sign handling.',
  },
  {
    ...
    virtual_address: '0x8018B1A0',
    signature: 'void FEngine::Update(long time_delta, unsigned int flags)',
    description: 'Core UI engine update step dispatching timers, animations, and input.',
    hint: 'Register swap between r30 and r31 in the joypad decrement loop (lines 301-304).',
  },
  {
    ...
    virtual_address: '0x800BB950',
    signature: 'int EAXCommon::Play(eMenuSoundTriggers trigger)',
    description: 'Plays frontend menu sound triggers via EAX dynamic sound mixer.',
    hint: 'Branch condition ordering and early exit logic when Debug_Common_FE_OFF is set.',
  },
  {
    ...
    virtual_address: '0x802EA1B8',
    signature: 'void CarPartCuller::CullParts(bVector3 *camera_pos, unsigned short flags)',
    description: 'Performs frustum and distance culling for vehicle parts against camera view.',
    hint: '99.4% match! Only a register swap between r17 and r18 around culldiv float load (lines 281-298).',
  },
]

log('Starting Batch 5 parallel decomp for ' + CANDIDATES.length + ' candidates across @nfs2 units')
...
const decompResults = await parallel(CANDIDATES.map((c) => () =>
  agent(
    ...
    '   - Register allocation differences (order of local variables, register asm hint if needed)\n' +
...
for (const result of succeeded) {
  ...
  const mr = await agent(
    ...
    '4. ninja -j2 (verify build passes)\n' +
    '5. ninja changes (verify no regressions)\n' +
    '6. If clean:\n' +
...
if (mergedFunctions.length > 0) {
  ...
  await agent(
    ...
    '   git commit -m "docs: document Batch 5 decompiled functions (@nfs2)\n\nCo-Authored-By: Claude Fable 5 <noreply@anthropic.com>"\n' +
    '   git push origin main',
    {
      phase: 'Docs',
      label: 'docs:update',
