export const meta = {
  name: 'nfs2-batch6-decomp',
  description: 'Batch 6 decompilation for @nfs2 units: bASin (zBWare), FEListBox::ScrollSelection (zFEng), EAXSound::GetCurMusicVolume (zEAXSound), CarLoader::SetMemoryPoolSize (zWorld)',
  phases: [
...
const CANDIDATES = [
  ...
  },
  {
    ...
    description: 'Scrolls listbox selection indices and computes element offsets.',
    hint: 'Register allocation for listbox dimensions and bounds checking.',
  },
  {
    demangled: 'EAXSound::GetCurMusicVolume',
    symbol: 'GetCurMusicVolume__8EAXSound',
    unit: 'main/Speed/Indep/SourceLists/zEAXSound',
    unit_obj: 'build/GOWE69/src/Speed/Indep/SourceLists/zEAXSound.o',
    size: 52,
    match: 0.0,
    source_file: 'src/Speed/Indep/Src/EAXSound/EAXSound.cpp',
    virtual_address: '0x800A6DAC',
    signature: 'float EAXSound::GetCurMusicVolume()',
    description: 'Returns current music volume by multiplying MasterVol with frontend or in-game music volume.',
    hint: 'Uncomment lines 466-473 in EAXSound.cpp: if (this->m_eSndGameMode == 1) return this->m_pCurAudioSettings->MasterVol * this->m_pCurAudioSettings->FEMusicVol; return this->m_pCurAudioSettings->MasterVol * this->m_pCurAudioSettings->IGMusicVol; (or update GetMasteredFEMusicVol/GetMasteredIGMusicVol in AudioSettings if needed). Target is exactly 13 instructions (52 bytes).',
  },
  {
...
log('Starting Batch 6 parallel decomp for ' + CANDIDATES.length + ' candidates across @nfs2 units')
...
if (mergedFunctions.length > 0) {
  ...
  await agent(
    ...
    '   git commit -m "docs: document Batch 6 decompiled functions (@nfs2)\n\nCo-Authored-By: Claude Fable 5 <noreply@anthropic.com>"\n' +
