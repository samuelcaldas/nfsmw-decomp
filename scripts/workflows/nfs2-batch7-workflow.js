export const meta = {
  name: 'nfs2-batch7-decomp',
  description: 'Batch 7 decompilation for @nfs2 units: bATan (zBWare), FEngine::UpdateMouseState (zFEng), EAXCommon::Play (zEAXSound), CarPartCuller::CullParts (zWorld)',
  phases: [
...
const CANDIDATES = [
  {
    ...
    description: 'Computes arctangent using quadrant classification and piecewise interpolation table.',
    hint: 'Line 330: replace register int quad asm("r6") = 0; with register int quad asm("r7") = 0; or plain int quad = 0;. Check table lookup indexing and branch logic.',
  },
  {
    ...
    signature: 'void FEngine::UpdateMouseState(FEPackage *pkg, FEObjectMouseState *state, float mx, float my)',
    description: 'Updates mouse pointer hover/click state on frontend packages and queues messages.',
    hint: 'Cache cFEng::Get() and pkg->name into local variables before if (is_mouse_over) to match target instruction scheduling and register assignment.',
  },
  {
    ...
    description: 'Plays frontend menu sound event with master volume and slider attenuation.',
    hint: 'Store this->m_pSFXOBJ_FEHUD in a local variable SFXObj_FEHUD *hud = this->m_pSFXOBJ_FEHUD; to avoid redundant reloads and match register r3 allocation.',
  },
  {
    ...
    description: 'Applies culling planes and steering angles to determine vehicle part visibility.',
    hint: '99.35% match. Examine loop induction variables, Polarity checks (CULLING_POLARITY_ANY_VISIBLE / CULLING_POLARITY_ALL_MUST_BE_VISIBLE) to eliminate remaining register differences.',
  },
]

log('Starting Batch 7 parallel decomp for ' + CANDIDATES.length + ' candidates across @nfs2 units')
...
if (mergedFunctions.length > 0) {
  ...
  await agent(
    ...
    '   git commit -m "docs: document Batch 7 decompiled functions (@nfs2)\n\nCo-Authored-By: Claude Fable 5 <noreply@anthropic.com>"\n' +
