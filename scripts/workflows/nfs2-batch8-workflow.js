export const meta = {
  name: 'nfs2-batch8-decomp',
  description: 'Batch 8 decompilation for @nfs2 units: fDeterminant (zBWare), FEngine::Update (zFEng), EAXTunerCar::UpdateRotation (zEAXSound), CarRenderInfo::UpdateWheelYRenderOffset (zWorld)',
  phases: [
...
const CANDIDATES = [
  {
    ...
    description: 'Computes 4x4 matrix determinant using cofactor expansion.',
    hint: 'Review 2x2/3x3 subdeterminants and grouping of float multiplications. Check register allocation order and temporary accumulation order.',
  },
  {
    ...
    description: 'Main per-frame update for frontend engine managing package ticks, script playback, and mouse updates.',
    hint: 'Check loop induction variable order and early exit conditions. Align temporary flags and pointer dereferences.',
  },
  {
    ...
    description: 'Updates tuner car rotation audio parameter with 0..1024 clamping.',
    hint: 'Exactly 10 instructions (40 bytes). Target assembly: mr r11, r3; li r9, 0; li r0, 0; mr r3, r9; stw r0, 0xc0(r11); cmpwi r9, 0x400; ble 0x875c; li r3, 0x400; stw r3, 0xc0(r11); blr. Implement to 100% byte-for-byte.',
  },
  {
    ...
    description: 'Computes vertical wheel offsets for suspension rendering and ride height simulation.',
    hint: '95.2% match. Align local array offsets and RideInfo attribute lookups. Match float conversion and clamping instructions.',
  },
]

log('Starting Batch 8 parallel decomp for ' + CANDIDATES.length + ' candidates across @nfs2 units')
...
if (mergedFunctions.length > 0) {
  ...
  await agent(
    ...
    '   git commit -m "docs: document Batch 8 decompiled functions (@nfs2)\n\nCo-Authored-By: Claude Fable 5 <noreply@anthropic.com>"\n' +
