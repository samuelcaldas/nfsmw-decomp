export const meta = {
  name: 'nfsmw-parallel-decomp-batch10-nfs3',
  description: 'Batch 10 decompilation for nfs3: cFEng::PushErrorPackage (zFe), CARSFX_PreColWoosh::MsgBarrier (zEAXSound2), GinsuSynthData::BindToData (zEAXSound2), IJoyHelper::EmulateMemoryCardLibrary (zFe)',
  phases: [
    { title: 'Decomp-1', detail: 'Pair 1: cFEng::PushErrorPackage (zFe) & CARSFX_PreColWoosh::MsgBarrier (zEAXSound2)' },
    { title: 'Decomp-2', detail: 'Pair 2: GinsuSynthData::BindToData (zEAXSound2) & IJoyHelper::EmulateMemoryCardLibrary (zFe)' },
...
const CANDIDATES_PAIR1 = [
  {
    ...
    extra_instructions: 'In FEngInterface.cpp cFEng::PushErrorPackage: inspect diff using python3 tools/decomp-diff.py -u main/Speed/Indep/SourceLists/zFe -d PushErrorPackage__5cFEngPCciUl. The function currently has duplicate branches under if (FEDatabase == nullptr) and else. Align the logic and condition checks to match the target assembly. Aim for 100.0% match parity. Ensure 0 regressions with ninja changes.',
  },
  {
    ...
    extra_instructions: 'In CARSFX_PreColWoosh.cpp CARSFX_PreColWoosh::MsgBarrier: inspect diff using python3 tools/decomp-diff.py -u main/Speed/Indep/SourceLists/zEAXSound2 -d MsgBarrier__18CARSFX_PreColWooshRC16MAudioReflection. Currently at 94.3% with only an instruction swap around line 57-58: li r4, 1 is emitted before stfs f13, 0x34(r10) in target binary. Reorder or declare local variables to produce the exact target sequence. Aim for 100.0% match. Verify with ninja and decomp-diff.',
  },
]
...
const CANDIDATES_PAIR2 = [
  {
    ...
    extra_instructions: 'In ginsudata.cpp GinsuSynthData::BindToData: inspect diff using python3 tools/decomp-diff.py -u main/Speed/Indep/SourceLists/zEAXSound2 -d BindToData__14GinsuSynthDataPv. Currently at 95.8% match. The differences are in lines 143-158 for member store ordering and loop counter initialization. Align local pointer variables and member assignments to match the original register allocation and instruction sequence. Aim for 100.0% match.',
  },
  {
    ...
    extra_instructions: 'In MemoryCardCallbacks.cpp IJoyHelper::EmulateMemoryCardLibrary: inspect diff using python3 tools/decomp-diff.py -u main/Speed/Indep/SourceLists/zFe -d EmulateMemoryCardLibrary__10IJoyHelperi. Currently at 97.5% match. The diff shows minor differences in stack offset store order for local pointers (r8, r9, r10, r11 at 8(r1), 0xc, 0x10, 0x14) and switch statement dispatch condition order. Align local variable setup to eliminate the differences. Aim for 100.0% match. Verify 0 regressions with ninja changes.',
  },
]
...
log('Starting Batch 10 for nfs3: cFEng::PushErrorPackage, CARSFX_PreColWoosh::MsgBarrier, GinsuSynthData::BindToData, IJoyHelper::EmulateMemoryCardLibrary')
