export const meta = {
  name: 'nfsmw-parallel-decomp-batch11-nfs3',
  description: 'Batch 11 decompilation for nfs3: PauseMenu::NotificationMessage (zFe), HandlePacketRelease (zEAXSound2), SuspensionSimple::Tire::UpdateLoaded (zPhysicsBehaviors), ShiftPoints (zPhysics)',
  phases: [
    { title: 'Decomp-1', detail: 'Pair 1: PauseMenu::NotificationMessage (zFe) & HandlePacketRelease (zEAXSound2)' },
    { title: 'Decomp-2', detail: 'Pair 2: SuspensionSimple::Tire::UpdateLoaded (zPhysicsBehaviors) & ShiftPoints (zPhysics)' },
...
const CANDIDATES_PAIR1 = [
  {
    ...
    virtual_address: '0x8011E0C8',
    extra_instructions: 'In uiPause.cpp PauseMenu::NotificationMessage: inspect diff using python3 tools/decomp-diff.py -u main/Speed/Indep/SourceLists/zFe -d NotificationMessage__9PauseMenuUlP8FEObjectUlUl. The switch statement on msg compares against message hash constants (0x9120409e, 0x43da9fd0, 0x30eb...). Align case statement ordering and branching so the compiler generates matching comparison binary search / dispatch trees. Aim for 100.0% match parity. Ensure 0 regressions with ninja changes.',
  },
  {
    ...
    extra_instructions: 'In ginsusynth.cpp GinsuSynthesis::HandlePacketRelease: inspect diff using python3 tools/decomp-diff.py -u main/Speed/Indep/SourceLists/zEAXSound2 -d HandlePacketRelease__14GinsuSynthesisPs. At line 96-102, diff shows ordering differences in loop counter comparison li r7, 0 / cmpw r7, r10 vs float constant loading and int-to-float conversions (xoris r0, r10, 0x8000). Align the sequence of variable declarations and loop initialization. Aim for 100.0% match. Verify with ninja and decomp-diff.',
  },
]
...
const CANDIDATES_PAIR2 = [
  {
    ...
    extra_instructions: 'In SuspensionSimple.cpp SuspensionSimple::Tire::UpdateLoaded: inspect diff using python3 tools/decomp-diff.py -u main/Speed/Indep/SourceLists/zPhysicsBehaviors -d UpdateLoaded__Q216SuspensionSimple4Tireffffff. Around lines 466-467, float multiplication sequencing on f13, f9, f10 vs f12, f0 has minor differences due to intermediate expressions. Reorder expressions in the tire load / slip calculations to match target instruction scheduling. Aim for 100.0% match.',
  },
  {
    ...
    extra_instructions: 'In PhysicsInfo.cpp Physics::Info::ShiftPoints: inspect diff using python3 tools/decomp-diff.py -u main/Speed/Indep/SourceLists/zPhysics -d ShiftPoints__Q27Physics4InfoRCQ36Attrib3Gen12transmissionRCQ36Attrib3Gen6engineRCQ36Attrib3Gen9inductionPfT4Ui. Currently at 90.4% match. The diff indicates register allocation shift (stmw r18 vs stmw r17) and branch label alignments in the gear loop. Adjust local loop variables and temporary floats to recover original register mapping. Aim for 100.0% match. Verify 0 regressions with ninja changes.',
  },
]
...
log('Starting Batch 11 for nfs3: PauseMenu::NotificationMessage, GinsuSynthesis::HandlePacketRelease, SuspensionSimple::Tire::UpdateLoaded, Physics::Info::ShiftPoints')
