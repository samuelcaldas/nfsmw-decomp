export const meta = {
  name: 'nfsmw-parallel-decomp-batch7-nfs3',
  description: 'Batch 7 decompilation for nfs3: Physics::Info::ShiftPoints (zPhysics), cFEng::PushErrorPackage (zFe), SuspensionTraffic::Tire::UpdateLoaded (zPhysicsBehaviors), GinsuSynthesis::HandlePacketRelease (zEAXSound2)',
  phases: [
    { title: 'Decomp-1', detail: 'Pair 1: Physics::Info::ShiftPoints (zPhysics) & cFEng::PushErrorPackage (zFe)' },
    { title: 'Decomp-2', detail: 'Pair 2: SuspensionTraffic::Tire::UpdateLoaded (zPhysicsBehaviors) & GinsuSynthesis::HandlePacketRelease (zEAXSound2)' },
...
const CANDIDATES_PAIR1 = [
  {
    ...
    extra_instructions: 'In PhysicsInfo.cpp Physics::Info::ShiftPoints: study diff using python3 tools/decomp-diff.py -u main/Speed/Indep/SourceLists/zPhysics -d ShiftPoints__Q27Physics4InfoRCQ36Attrib3Gen12transmissionRCQ36Attrib3Gen6engineRCQ36Attrib3Gen9inductionPfT4Ui. The original binary saves r17..r31 with stmw r17, 0xc(r1) whereas decomp saves r18..r31 with stmw r18, 0x10(r1). Introduce a variable or bind a register (such as asm("r17")) to align the saved register set, bringing r21 vs r22, r23 vs r24, r24 vs r25, r26 vs r27 into exact alignment. Aim for highest match without regressions. Verify with ninja and decomp-diff.',
  },
  {
    ...
    extra_instructions: 'In FEngInterface.cpp cFEng::PushErrorPackage: study diff using python3 tools/decomp-diff.py -u main/Speed/Indep/SourceLists/zFe -d PushErrorPackage__5cFEngPCciUl. Note that in original code: if (FEDatabase == nullptr) { if (cFEng::Get()->IsPackagePushed(pPackageName)) return; } then the rest of package creation is shared (FEPackageManager::Get()->SetPackageDataArg, mFEng->PushPackage with FE_PACKAGE_PRIORITY_ERROR, SetErrorScreen(true), ToggleErrorScreenMode(true), and pause check). Reorganize to eliminate duplicated branch code. Aim for 100% match. Verify with ninja and decomp-diff.',
  },
]
...
const CANDIDATES_PAIR2 = [
  {
    ...
    extra_instructions: 'In SuspensionTraffic.cpp SuspensionTraffic::Tire::UpdateLoaded: study diff using python3 tools/decomp-diff.py -u main/Speed/Indep/SourceLists/zPhysicsBehaviors -d UpdateLoaded__Q217SuspensionTraffic4Tireffff. The original binary saves r30 and r31 using stmw r30, 0x10(r1) and has stack size 0x30, keeping a constant pointer in r30 across calls to VU0_Atan2. Assign a local variable or register (e.g. register const float *kAtanConst asm("r30") or float threshold) to preserve r30 and match stack size 0x30. Verify 0 regressions with ninja changes.',
  },
  {
    ...
    extra_instructions: 'In ginsusynth.cpp GinsuSynthesis::HandlePacketRelease: study diff using python3 tools/decomp-diff.py -u main/Speed/Indep/SourceLists/zEAXSound2 -d HandlePacketRelease__14GinsuSynthesisPs. In the loop around line 100-107, refine the loop counter variable (e.g. register int i asm("r7")) and float conversion order so instruction scheduling and sthx sequencing match. Verify with ninja and decomp-diff.',
  },
]
...
log('Starting Batch 7 for nfs3: Physics::Info::ShiftPoints, cFEng::PushErrorPackage, SuspensionTraffic::Tire::UpdateLoaded, GinsuSynthesis::HandlePacketRelease')
