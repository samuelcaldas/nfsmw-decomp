export const meta = {
  name: 'nfsmw-parallel-decomp-batch9-nfs3',
  description: 'Batch 9 decompilation for nfs3: MemcardCallbacks::FoundEntry (zFe), SFX_Common::MsgPlayMiscSound (zEAXSound2), CARSFX_RoadNoise::Play (zEAXSound2), SuspensionTraffic::Tire::UpdateLoaded (zPhysicsBehaviors)',
  phases: [
    { title: 'Decomp-1', detail: 'Pair 1: MemcardCallbacks::FoundEntry (zFe) & SFX_Common::MsgPlayMiscSound (zEAXSound2)' },
    { title: 'Decomp-2', detail: 'Pair 2: CARSFX_RoadNoise::Play (zEAXSound2) & SuspensionTraffic::Tire::UpdateLoaded (zPhysicsBehaviors)' },
...
const CANDIDATES_PAIR1 = [
  {
    ...
    extra_instructions: 'In MemoryCardCallbacks.cpp MemcardCallbacks::FoundEntry: study diff using python3 tools/decomp-diff.py -u main/Speed/Indep/SourceLists/zFe -d FoundEntry__16MemcardCallbacksPCQ211RealmcIface9EntryInfo. Notice the function is at 98.0% match with only an instruction scheduling difference around UIMemcardBase::AddItem (line 261): lis r5, $LC71@ha is emitted before lwz r4, 0(r31) instead of after. Adjust expression ordering or use local variables to match exact instruction schedule. Aim for 100.0% match. Verify with ninja and decomp-diff.',
  },
  {
    ...
    size: 780,
    match: 99.39,
    source_file: 'src/Speed/Indep/Src/EAXSound/SFX/SFX_Common.cpp',
    extra_instructions: 'In SFX_Common.cpp SFX_Common::MsgPlayMiscSound: study diff using python3 tools/decomp-diff.py -u main/Speed/Indep/SourceLists/zEAXSound2 -d MsgPlayMiscSound__10SFX_CommonRC10MMiscSound. The function is at 99.4% match with minor differences in member zeroing and register assignment order in the Csis::Class::CreateInstance setup blocks. Reorder struct member writes to match original store offsets (stw r31, 4(r30), 8, 0xc, 0x10, 0x14). Aim for 100.0% match. Verify with ninja and decomp-diff.',
  },
]
...
const CANDIDATES_PAIR2 = [
  {
    ...
    extra_instructions: 'In CARSFX_RoadNoise.cpp CARSFX_RoadNoise::Play: study diff using python3 tools/decomp-diff.py -u main/Speed/Indep/SourceLists/zEAXSound2 -d Play__16CARSFX_RoadNoise16FXROADNOISE_LOOPi. Study lines 167-339: register assignment order for r8, r9, r10, r11 and member stores at offsets 4, 8, 0xc, 0x10, 0x14, 0x18, 0x1c, 0x20, 0x24, 0x28, 0x2c. Align the struct initialization to eliminate differences. Aim for 100.0% match. Verify with ninja and decomp-diff.',
  },
  {
    ...
    extra_instructions: 'In SuspensionTraffic.cpp SuspensionTraffic::Tire::UpdateLoaded: study diff using python3 tools/decomp-diff.py -u main/Speed/Indep/SourceLists/zPhysicsBehaviors -d UpdateLoaded__Q217SuspensionTraffic4Tireffff. Currently at 97.2% match. The original binary saves r30 as a callee-saved register (stmw r30, 0x10(r1), frame size 0x30) to preserve a pointer across the VU0_Atan2 call. Ensure register r30 is used across calls or introduce an intermediate variable so GCC generates stmw r30, 0x10(r1). Verify 0 regressions with ninja changes.',
  },
]
...
log('Starting Batch 9 for nfs3: MemcardCallbacks::FoundEntry, SFX_Common::MsgPlayMiscSound, CARSFX_RoadNoise::Play, SuspensionTraffic::Tire::UpdateLoaded')
